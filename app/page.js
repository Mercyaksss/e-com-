...
export default function SoleLanding() {
  ...
  return (
    <div className="bg-[#0a0a0a] text-[#f5f0eb] overflow-x-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-14 py-7 mix-blend-difference">
      ...
      <section className="min-h-screen grid md:grid-cols-2 relative overflow-hidden">
        ...
        <div
          ref={heroRef}
          className={`reveal ${heroVisible ? 'visible' : ''} flex flex-col justify-center px-14 pt-40 pb-20 relative z-10`}
        >
          <span className="flex items-center gap-3 text-[#e8530a] text-xs tracking-[0.3em] uppercase mb-8">
            <span className="w-8 h-px bg-[#e8530a]" />
            New Collection 2025
          </span>
          <h1 className="font-bebas leading-[0.9] tracking-wide mb-8" style={{ fontSize: 'clamp(5rem, 10vw, 10rem)' }}>
            Step<br />Into<br />
            <span className="text-[#e8530a]">Style</span>
          </h1>
          <p className="text-[#888] max-w-xs leading-relaxed mb-12 font-light">
            Premium footwear from the world's leading brands. Engineered for performance, designed to turn heads.
          </p>
          <div className="flex items-center gap-6">
            <a
              href="/productspage"
              className="clip-btn bg-[#e8530a] text-[#f5f0eb] px-11 py-[18px] text-sm font-medium tracking-[0.15em] uppercase hover:bg-[#ff6b2b] transition-all hover:-translate-y-0.5 no-underline"
            >
              Shop Now
            </a>
        ...
        <div className="hidden md:flex relative overflow-hidden items-center justify-center">
          ...
          <div className="animate-float text-[22rem] drop-shadow-[0_40px_80px_rgba(232,83,10,0.3)] select-none">👟</div>
          <span className="absolute bottom-14 right-14 font-bebas text-[8rem] text-white/[0.04] leading-none select-none">01</span>
        </div>

        {/* Scroll indicator */}
        <div className="absolute bottom-10 left-14 flex items-center gap-3 text-xs tracking-[0.25em] uppercase text-[#888]">
      ...
      {/* MARQUEE */}
      <div className="bg-[#e8530a] py-4 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          {[...marqueeItems, ...marqueeItems].map((item, i) => (
            <span key={i} className="font-bebas text-2xl tracking-widest text-[#0a0a0a] px-10 after:content-['✦'] after:ml-10 after:text-sm">
              {item}
            </span>
          ))}
        </div>
      ...
      {/* BRANDS */}
      <div id="brands" className="bg-[#1a1a1a] py-16 px-14">
        <p className="text-center text-xs tracking-[0.35em] uppercase text-[#888] mb-12">Trusted by sneakerheads worldwide</p>
        <div className="flex flex-wrap items-center justify-center gap-14">
          {brands.map(b => (
            <span key={b} className="font-bebas text-2xl tracking-widest text-[#888] hover:text-[#f5f0eb] transition-colors cursor-default">{b}</span>
          ))}
        </div>
      </div>
      ...
      <section
        id="shop"
        ref={productsRef}
        className={`reveal ${productsVisible ? 'visible' : ''} bg-[#0a0a0a] px-14 py-28`}
      >
        {/* Header */}
          ...
          <div className="flex gap-3 flex-wrap">
            {filters.map(f => (
              <button
                ...
                className={`px-5 py-2.5 border text-xs tracking-[0.1em] uppercase transition-all cursor-pointer
                  ${activeFilter === f
                    ? 'border-[#e8530a] text-[#e8530a]'
                    : 'border-[#2e2e2e] text-[#888] hover:border-[#e8530a] hover:text-[#e8530a]'
                  }`}
              >
                {f}
              </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-0.5">
          {products.map((p) => (
            <div
              key={p.id}
              className={`bg-[#1a1a1a] relative overflow-hidden group ${p.span ? 'md:col-span-2' : ''}`}
            >
              {p.badge && (
              ...
              <button
                ...
                className={`absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center text-xl text-white opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-200 cursor-pointer
                  ${added[p.id] ? 'bg-green-500' : 'bg-[#e8530a]'}`}
              >
                {added[p.id] ? '✓' : '+'}
              </button>
              <div className={`flex items-center justify-center overflow-hidden transition-transform duration-500 group-hover:scale-105 relative
                ${p.span ? 'text-[12rem] aspect-[2/1]' : 'text-[7rem] aspect-square'}`}>
              ...
              <div className="px-7 py-5 border-t border-[#2e2e2e] flex justify-between items-end">
                <div>
      ...
      <section
        id="about"
        ref={featuresRef}
        className={`reveal ${featuresVisible ? 'visible' : ''} bg-[#1a1a1a] px-14 py-28 grid md:grid-cols-2 gap-20 items-center`}
      >
        {/* Visual */}
        <div className="hidden md:flex items-center justify-center h-[500px]">
          <div className="relative w-[380px] h-[380px] animate-spin-slow">
            ...
            {/* Dots */}
            {[['top-[-5px] left-1/2', ''], ['bottom-[-5px] left-1/2', ''], ['left-[-5px] top-1/2', ''], ['right-[-5px] top-1/2', '']].map((_, i) => (
              <span key={i} className={`absolute w-2.5 h-2.5 rounded-full bg-[#e8530a] ${['top-[-5px] left-[calc(50%-5px)]', 'bottom-[-5px] left-[calc(50%-5px)]', 'left-[-5px] top-[calc(50%-5px)]', 'right-[-5px] top-[calc(50%-5px)]'][i]}`} />
            ))}
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-[9rem] animate-spin-slow-rev drop-shadow-[0_20px_40px_rgba(232,83,10,0.4)] select-none">👟</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <h2 className="font-bebas leading-none mb-14" style={{ fontSize: 'clamp(3rem, 6vw, 5.5rem)' }}>
            Built<br />Different
          </h2>
          <div className="flex flex-col gap-10">
            {features.map(f => (
              <div key={f.num} className="flex gap-6 items-start group">
                <span className="font-bebas text-5xl text-white/[0.08] leading-none min-w-[56px] group-hover:text-[#e8530a] transition-colors">
                  {f.num}
                </span>
                <div>
      ...
      <section
        ref={testimonialsRef}
        className={`reveal ${testimonialsVisible ? 'visible' : ''} bg-[#0a0a0a] px-14 py-28`}
      >
        ...
        <div className="grid md:grid-cols-3 gap-0.5">
          {testimonials.map((t) => (
            <div key={t.name} className="bg-[#1a1a1a] p-10 relative overflow-hidden">
      ...
      <section
        id="contact"
        ref={ctaRef}
        className={`reveal ${ctaVisible ? 'visible' : ''} bg-[#e8530a] px-14 py-24 grid md:grid-cols-2 gap-14 items-center relative overflow-hidden`}
      >
        <span className="absolute right-[-40px] top-1/2 -translate-y-1/2 font-bebas text-[22rem] text-black/[0.08] leading-none select-none pointer-events-none">SOLE</span>
        ...
        <div className="relative z-10">
          <p className="text-black/60 leading-relaxed font-light mb-9">
            Browse over 500 styles from the world's biggest brands. New drops every Friday. Free shipping on orders over $100.
          </p>
          <a
            href="#shop"
            className="clip-btn bg-[#0a0a0a] text-[#f5f0eb] px-11 py-[18px] text-sm font-medium tracking-[0.15em] uppercase hover:bg-[#222] transition-all hover:-translate-y-0.5 inline-block no-underline"
          >
            Browse Collection
          </a>
        </div>
      ...
      {/* FOOTER */}
      <footer className="bg-[#1a1a1a] px-14 pt-20 pb-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-14 mb-14">
          ...
          {[
            ...
            <div key={col.title}>
              <h4 className="text-xs tracking-[0.3em] uppercase text-[#f5f0eb] mb-6">{col.title}</h4>
